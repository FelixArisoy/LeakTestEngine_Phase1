...
function clampStationCount(value: any): number {
...
function getControl(id: string): any {
    return TcHmi.Controls.get(id) as any;
}
...
function bindDataSymbol(controlId: string, symbolExpression: string) {
...
function getControlElement(control: any): HTMLElement | null {
    if (!control || typeof control.getElement !== 'function') return null;

    var elementList = control.getElement();
    if (!elementList || !elementList[0]) return null;

    return elementList[0] as HTMLElement;
}
...
function getParentTableCell(element: any): HTMLElement | null {
    var current = element;

    while (current) {
        if (current.tagName === 'TD') return current as HTMLElement;
...
function markStationTab(index: number, selected: boolean) {
...
function writeBoolField(symbol: string, value: any) {
...
function writeBoolInArray(symbolExpression: string, rowIndex: number, value: any) {
    var boolValue = toBool(value);

    TcHmi.Symbol.readEx2<any[]>(symbolExpression, function (readData) {
...
function attachToggleDomWriteBack(gridId: string, symbolExpressionGetter: () => string) {
    var grid = getControl(gridId);
    var element = getControlElement(grid);

    if (!element || (element as any).__valiantToggleWriteBackAttached) return;

    (element as any).__valiantToggleWriteBackAttached = true;
...
function registerEnableWriteBack(gridId: string, symbolExpressionGetter: () => string) {
    TcHmi.EventProvider.register(gridId + '.onDataChanged', function (_event, change) {
        if (!change || change.property !== 'bEnabled') return;
...
new TcHmi.Symbol<number>(stationCountExpr).watch(function (data) {
